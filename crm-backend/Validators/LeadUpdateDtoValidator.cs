...
public class LeadUpdateDtoValidator : AbstractValidator<UpdateLeadDto>
{
    public LeadUpdateDtoValidator()
    {
        ...
        RuleFor(x => x.strPhone)
            .MaximumLength(20).When(x => !string.IsNullOrEmpty(x.strPhone));

        RuleFor(x => x.strCompanyName)
            .MaximumLength(200).When(x => !string.IsNullOrEmpty(x.strCompanyName));

        RuleFor(x => x.strJobTitle)
            .MaximumLength(150).When(x => !string.IsNullOrEmpty(x.strJobTitle));
        ...
        RuleFor(x => x.strStatus)
            ...
            .Must(status => LeadStatusConstants.AllStatuses.Contains(status))
            .WithMessage($"Status must be one of: {string.Join(", ", LeadStatusConstants.AllStatuses)}");
