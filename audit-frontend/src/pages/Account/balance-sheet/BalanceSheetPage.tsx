...
import {
  useBalanceSheetWithAccounts,
  useExportBalanceSheetPdf,
  useExportBalanceSheetExcel,
  useExportBalanceSheetCsv,
  usePreviewBalanceSheetPdf,
} from "@/hooks/api/Account/use-balance-sheet";
import type {
  BalanceSheetFilter,
  BalanceSheetScheduleNode,
  BalanceSheetAccount,
...
interface FlatBalanceSheetItem {
  id: string;
  name: string;
  code: string;
  level: number;
...
const flattenBalanceSheetTree = (
  tree: BalanceSheetScheduleNode[]
): FlatBalanceSheetItem[] => {
  const result: FlatBalanceSheetItem[] = [];

  const processNode = (
    node: BalanceSheetScheduleNode,
    ...
    // Add accounts
    node.accounts.forEach((account: BalanceSheetAccount) => {
      result.push({
    ...
    // Process children
    node.children.forEach((child: BalanceSheetScheduleNode) => {
...
const BalanceSheetPage: React.FC = () => {
  ...
  const {
    ...
    refetch,
  } = useBalanceSheetWithAccounts(submittedFilter, false);

  const exportPdfMutation = useExportBalanceSheetPdf();
  const exportExcelMutation = useExportBalanceSheetExcel();
  const exportCsvMutation = useExportBalanceSheetCsv();
  const previewPdfMutation = usePreviewBalanceSheetPdf();
  ...
  const flattenedData = useMemo(() => {
    if (!report?.scheduleTree) return [];
    return flattenBalanceSheetTree(report.scheduleTree);
  }, [report?.scheduleTree]);
  ...
  return (
    <CustomContainer>
      <PageHeader
        title="Balance Sheet"
        description="View balance sheet with opening balance, period movements, and closing balance."
        icon={PieChart}
      />
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <WithPermission
                module={ModuleBase.BALANCE_SHEET}
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <Card>
                ...
                <CardContent className="text-2xl font-semibold">
                  {formatNumber(report?.dblTotalAssets)}
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="py-3">
                ...
                <CardContent className="text-2xl font-semibold">
                  {formatNumber(report?.dblTotalLiabilities)}
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="py-3">
                ...
                <CardContent className="text-2xl font-semibold">
                  {formatNumber(report?.dblTotalCapital)}
                </CardContent>
              </Card>
            </div>
            <SearchInput
