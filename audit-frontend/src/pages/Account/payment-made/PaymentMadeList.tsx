...
import { usePaymentMade } from "@/hooks/api/Account/use-payment-made";
...
const PaymentMadeList: React.FC = () => {
  ...
  const HeaderIcon = useMenuIcon(ListModules.PAYMENT_MADE, Receipt);
  ...
  const {
    ...
    setColumnWidths,
    resetAll,
  } = useTableLayout("payment_made", defaultColumnOrder, []);

  const { data: paymentMadeResponse, isLoading } = usePaymentMade({
  ...
  const columns = useMemo<DataTableColumn<PaymentMade>[]>(() => {
    const baseColumns: DataTableColumn<PaymentMade>[] = [];

    if (canAccess(menuItems, FormModules.PAYMENT_MADE, Actions.EDIT)) {
      baseColumns.push({
        key: "actions",
        header: "Actions",
        width: "80px",
        cell: (item) => (
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                openEditInNewTab(`/payment-made/${item.strPaymentMadeGUID}`);
              }}
      ...
      },
      {
        ...
        cell: (item) => {
          try {
            ...
            }).format(item.dblTotalAmountMade);
          } catch {
            return `INR ${item.dblTotalAmountMade.toFixed(2)}`;
          }
        },
        sortable: true,
      },
      {
        key: "dblTotalAmountMadeBase",
        header: "Base Amount",
        width: "150px",
        cell: (item) => {
          try {
            return new Intl.NumberFormat("en-IN", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            }).format(item.dblTotalAmountMadeBase);
          } catch {
            return item.dblTotalAmountMadeBase.toFixed(2);
          }
        },
      },
      {
  ...
  return (
    <CustomContainer>
      <PageHeader
        title="Payment Made"
        description="Manage all payment made records"
        icon={HeaderIcon}
        actions={
          <WithPermission
          ...
          >
            <Button
              onClick={() => navigate("/payment-made/new")}
        ...
        >
          <Card className="mt-4">
            <CardHeader>
              ...
              <CardDescription>
                Filter payment made records by additional criteria
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      {isLoading ? (
        ...
        <DataTable
          ...
          onColumnWidthsChange={(widths) => {
            ...
            localStorage.setItem(
              "payment_made_column_widths",
              JSON.stringify(widths)
            );
          }}
          emptyState={
            debouncedSearch ? (
              <>No payment made found matching "{debouncedSearch}".</>
            ) : (
              <>No payment made found. Click "Create New" to create one.</>
            )
          }
        />
      )}
