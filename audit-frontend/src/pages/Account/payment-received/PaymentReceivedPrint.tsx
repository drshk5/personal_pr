...
import { usePrintPaymentReceived } from "@/hooks/api/Account/use-payment-received";
...
interface PaymentReceivedPrintData {
  ...
  strCustomerCity?: string;
  strCustomerState?: string;
  strCustomerZipCode?: string;
  strCustomerCountry?: string;
...
const PaymentReceivedPrint: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { data: paymentData, isLoading } = usePrintPaymentReceived(id);
  const canPrint = usePermission(FormModules.PAYMENT_RECEIVED, Actions.PRINT);
  ...
  useEffect(() => {
    if (paymentData) {
      ...
      const receiptNo = (paymentData as PaymentReceivedPrintData)?.strPaymentReceivedNo || "Payment Receipt";
  ...
  const data = paymentData as PaymentReceivedPrintData;
    ...
    <CustomContainer>
      ...
      <div id="payment-print-area">
        ...
        >
          ...
          {/* Logo and Payment Receipt Title */}
          ...
          <div className="flex justify-between items-start mb-8">
            ...
            <div className="flex-1 text-base print:text-lg">
              <div className="mb-4">
                ...
                <div className="font-medium text-gray-900">
                  {formatDate(data.dPaymentReceivedDate)}
                </div>
              </div>
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                <div className="font-medium text-gray-900">
                  {data.strTotalAmountInWords || data.strAmountInWords || `${currencyCode} ${formatMoney(data.dblTotalAmountReceived)} Only`}
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <div className="bg-[#66ad37] text-white px-8 py-6 print:px-10 print:py-8 rounded-lg min-w-40">
                <div className="text-lg print:text-xl mb-2">Amount Received</div>
                <div className="text-2xl print:text-3xl font-bold flex items-baseline gap-2">
                  {formatMoney(data.dblTotalAmountReceived)}
          ...
          <div className="pt-4 mb-4">
            ...
            <div className="flex justify-between items-start">
              <div>
                <div className="text-lg print:text-xl mb-2">Received From</div>
                <div className="text-lg print:text-xl font-semibold">
                  {data.strCustomerName || data.strCustomerAccountName || "Customer"}
                </div>
                {(data.strCustomerAddress || data.strCustomerCity || data.strCustomerState || data.strCustomerZipCode || data.strCustomerCountry) && (
                  <div className="text-base print:text-lg text-gray-700 mt-1 leading-relaxed">
                    {data.strCustomerAddress && <div>{data.strCustomerAddress}</div>}
                    <div>
                      {[data.strCustomerCity, data.strCustomerState, data.strCustomerZipCode]
                        .filter(Boolean)
                        .join(", ")}
                    </div>
                    {data.strCustomerCountry && <div>{data.strCustomerCountry}</div>}
                  </div>
                )}
              </div>
              <div className="flex justify-end">
                <div className="text-center w-56 print:w-64">
                  <div className="h-16 print:h-20"></div>
                  <div className="border-t-2 border-gray-400 pt-1 text-base print:text-lg font-medium">
                ...
                <tbody className="divide-y divide-gray-200">
                  {data.items.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="p-3">
                        {formatDate(item.dtPaymentReceivedOn)}
                      </td>
...
export default PaymentReceivedPrint;
