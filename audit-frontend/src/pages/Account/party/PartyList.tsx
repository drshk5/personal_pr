...
import type { Party } from "@/types/Account/party";
...
import { useParties } from "@/hooks/api/Account/use-parties";
...
const PartyList: React.FC = () => {
  ...
  const HeaderIcon = useMenuIcon(ListModules.CUSTOMER, Users);
  ...
  const {
    ...
    setColumnWidths,
    resetAll,
  } = useTableLayout("party", defaultColumnOrder, []);
  ...
  useEffect(() => {
    if (partiesResponse?.data) {
      setPagination({
        pageNumber: partiesResponse.pageNumber,
        pageSize: partiesResponse.pageSize,
        totalCount: partiesResponse.totalRecords,
        totalPages: partiesResponse.totalPages,
      });
    }
  }, [partiesResponse, setPagination]);
  ...
  const columns = useMemo<DataTableColumn<Party>[]>(() => {
    const baseColumns: DataTableColumn<Party>[] = [];
        ...
        cell: (party) => (
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                openEditInNewTab(`/customer/${party.strPartyGUID}`);
              }}
              title="Edit customer"
    ...
    baseColumns.push(
      {
        ...
        cell: (party) => (
          <div
            ...
            title={party.strPartyName_Display}
          >
      ...
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strCompanyName || "-"}
          >
            {party.strCompanyName || "-"}
          </div>
        ),
        sortable: true,
      },
      ...
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strPhoneNoWork || "-"}
          >
            {party.strPhoneNoWork || "-"}
          </div>
        ),
        sortable: true,
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strPhoneNoPersonal || "-"}
          >
            {party.strPhoneNoPersonal || "-"}
          </div>
        ),
        sortable: true,
      },
      ...
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strPartyLanguage || "-"}
          >
            {party.strPartyLanguage || "-"}
          </div>
        ),
        sortable: true,
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strCurrencyTypeName || undefined}
          >
            {party.strCurrencyTypeName}
          </div>
        ),
        sortable: true,
      },
      ...
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strDesignation}
          >
            {party.strDesignation}
          </div>
        ),
        sortable: true,
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strCreatedByName || undefined}
          >
            {party.strCreatedByName}
          </div>
        ),
        sortable: true,
      },
      {
        ...
        cell: (party) => (
          <div className="whitespace-nowrap">
            {party.dtCreatedOn
              ? format(new Date(party.dtCreatedOn), "MMM d, yyyy, h:mm a")
              : "-"}
          </div>
        ),
        sortable: true,
      },
      {
        ...
        cell: (party) => (
          <div
            className={isTextWrapped ? "wrap-break-word" : "truncate"}
            title={party.strUpdatedByName || undefined}
          >
            {party.strUpdatedByName}
          </div>
        ),
        sortable: true,
      },
      {
        ...
        cell: (party) => (
          <div className="whitespace-nowrap">
            {party.dtUpdatedOn
              ? format(new Date(party.dtUpdatedOn), "MMM d, yyyy, h:mm a")
              : "-"}
          </div>
        ),
        sortable: true,
  ...
  return (
    <CustomContainer>
        ...
        >
          <Card className="mt-4">
            <CardHeader>
              ...
              <CardDescription>
                Filter customers by additional criteria
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      {isLoading ? (
        ...
        <DataTable<Party>
          data={error ? [] : partiesResponse?.data || []}
          ...
          onColumnWidthsChange={(widths) => {
            ...
            localStorage.setItem("party_column_widths", JSON.stringify(widths));
          }}
          keyExtractor={(item) => item.strPartyGUID}
          emptyState={
            error ? (
              <>An error occurred loading customers. Please try again later.</>
            ) : debouncedSearch ? (
