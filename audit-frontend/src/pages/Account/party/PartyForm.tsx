...
import { partySchema, type PartyFormValues } from "@/validations/Account/party";
...
import { PartyContactList } from "./party-contact/PartyContactList";
import { PartyContactModal } from "./party-contact/PartyContactModal";
import { PartyFormSkeleton } from "./PartyFormSkeleton";
...
const PartyForm: React.FC = () => {
  ...
  const HeaderIcon = useMenuIcon("customer_form", Users);
  ...
  const {
    data: party,
    isFetching: isFetchingParty,
    error: partyError,
  } = useParty(isEditMode ? id : undefined);
  ...
  React.useEffect(() => {
    if (party) {
      const resetValues = {
        strPartyType: "Customer",
        strSalutation: party.strSalutation || "",
        strFirstName: party.strFirstName || "",
        strLastName: party.strLastName || "",
        strCompanyName: party.strCompanyName || "",
        strPartyName_Display: party.strPartyName_Display || "",
        strUDFCode: party.strUDFCode || "",
        strEmail: party.strEmail || "",
        strPhoneNoWork: party.strPhoneNoWork || "",
        strPhoneNoPersonal: party.strPhoneNoPersonal || "",
        strPAN: party.strPAN || "",
        strCurrencyTypeGUID: party.strCurrencyTypeGUID || "",
        intPaymentTerms_inDays: party.intPaymentTerms_inDays || null,
        strPartyLanguage: party.strPartyLanguage || "",
        strTaxRegNo: party.strTaxRegNo || "",
        strWebsiteURL: party.strWebsiteURL || "",
        strDepartment: party.strDepartment || "",
        strDesignation: party.strDesignation || "",
        strTwitter: party.strTwitter || "",
        strSkype: party.strSkype || "",
        strFacebook: party.strFacebook || "",
        strInstagram: party.strInstagram || "",
        dblIntrest_per: party.dblIntrest_per || null,
        strRemarks: party.strRemarks || "",
        strAttention_billing: party.strAttention_billing || "",
        strCountryGUID_billing: party.strCountryGUID_billing || "",
        strAddress_billing: party.strAddress_billing || "",
        strStateGUID_billing: party.strStateGUID_billing || "",
        strCityGUID_billing: party.strCityGUID_billing || "",
        strPinCode_billing: party.strPinCode_billing || "",
        strPhone_billing: party.strPhone_billing || "",
        strFaxNumber_billing: party.strFaxNumber_billing || "",
        strAttention_shipping: party.strAttention_shipping || "",
        strCountryGUID_shipping: party.strCountryGUID_shipping || "",
        strAddress_shipping: party.strAddress_shipping || "",
        strStateGUID_shipping: party.strStateGUID_shipping || "",
        strCityGUID_shipping: party.strCityGUID_shipping || "",
        strPinCode_shipping: party.strPinCode_shipping || "",
        strPhone_shipping: party.strPhone_shipping || "",
        strFaxNumber_shipping: party.strFaxNumber_shipping || "",
      };
      ...
      form.setValue("strPartyType", "Customer", {
        shouldDirty: false,
        shouldTouch: false,
        shouldValidate: false,
      });

      form.setValue("strCurrencyTypeGUID", party.strCurrencyTypeGUID || "", {
        shouldDirty: false,
        shouldTouch: false,
        shouldValidate: false,
      });

      form.setValue("strPartyName_Display", party.strPartyName_Display || "", {
        shouldDirty: false,
        shouldTouch: false,
        shouldValidate: true,
      });
  ...
  const getDisplayNameSuggestions = () => {
    ...
    // Include current value if it exists (for edit mode)
  ...
  const onSubmit = async (values: PartyFormValues) => {
    if (isEditMode && id) {
      const partyUpdate: PartyUpdate = {
        ...
        dblIntrest_per: values.dblIntrest_per || null,
      ...
      if (result?.strPartyGUID) {
        ...
        navigate(`/customer/${result.strPartyGUID}`);
      } else {
        navigate("/customer");
      }
    }
  ...
  const handleAddSocialField = (fieldName: string) => {
    ...
    form.setValue(fieldName as keyof PartyFormValues, "");
    setShowSocialDropdown(false);
  };
  ...
  const handleCopyBillingAddress = (checked: boolean) => {
    ...
    if (checked) {
      // Copy all billing address fields to shipping
      form.setValue(
        "strAttention_shipping",
  ...
  if (
    isEditMode &&
    !isFetchingParty &&
    (partyError || (!party && !isFetchingParty))
  ) {
    return <NotFound pageName="Customer" />;
  }

  const isLoading = isFetchingParty || isLoadingCurrencyTypes;
  ...
  return (
    <CustomContainer>
      <PageHeader
        ...
        description={
          isEditMode
            ? `Editing customer ${party?.strPartyName_Display || ""}`
          ...
          <Form {...form}>
            <form
              onSubmit={form.handleSubmit((data) =>
                onSubmit(data as PartyFormValues)
              )}
              className="space-y-6"
            >
              <Card>
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent side="right" className="w-64">
                                    <p className="whitespace-normal">
                                      ...
                                      transactions you create for this customer.
                                    </p>
                                  </TooltipContent>
                                </Tooltip>
                              </FormLabel>
                        ...
                        {dynamicSocialFields.map((fieldName) => {
                          ...
                          return (
                            <FormField
                              key={fieldName}
                              control={form.control}
                              name={fieldName as keyof PartyFormValues}
                              render={({ field }) => (
                                <FormItem>
