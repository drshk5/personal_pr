        ...
        >
          ...
          <CommandList>
            ...
            <CommandGroup>
              {data.map((item) => (
                <CommandItem
                  className="capitalize text-foreground data-[disabled=true]:opacity-50 data-[disabled=true]:pointer-events-none"
                  key={item.value}
                  disabled={false}
                  aria-disabled={false}
...
export type CalendarBodyProps = {
  ...
  disableBefore?: Date; // render days before this as disabled (style and no click)
  onDayDrop?: (date: Date, draggedId: string) => void; // optional DnD drop handler per day
};
...
export const CalendarBody = ({
  ...
  const { startDay, disabled } = useContext(CalendarContext);
  ...
  // Normalize disableBefore to start of day
  const disableBeforeMs = disableBefore
    ? new Date(
        disableBefore.getFullYear(),
        disableBefore.getMonth(),
        disableBefore.getDate()
      ).getTime()
    : undefined;
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const isDisabled =
      disableBeforeMs !== undefined && dateForDay.getTime() < disableBeforeMs;

    days.push(
      <div
        className={cn(
          "relative flex h-full w-full flex-col gap-1 p-1 text-muted-foreground text-xs min-h-0",
        ...
        onDrop={(e) => {
          ...
          if (draggedId) {
            onDayDrop?.(new Date(year, month, day), draggedId);
          }
        }}
      >
        {day}
        <div className="flex-1 space-y-0.5 overflow-y-auto min-h-0 scrollbar-thin scrollbar-thumb-muted-foreground/30 scrollbar-track-transparent hover:scrollbar-thumb-muted-foreground/50">
          {featuresForDay.map((feature) => children({ feature }))}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="grid grow grid-cols-7">
      {days.map((day, index) => (
        <div
          className={cn(
            "relative aspect-square overflow-hidden border-t border-r border-border-color",
  ...
  ({ feature, className }: CalendarItemProps) => {
    ...
    return (
      <div
        ...
        onDragStart={(e) => {
          if (disabled) return;
          e.dataTransfer.setData("text/task-id", feature.id);
...
export const CalendarProvider = ({
  ...
  <CalendarContext.Provider value={{ locale, startDay, disabled }}>
